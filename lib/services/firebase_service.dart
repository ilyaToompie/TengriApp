...
class FirebaseService {
  ...
  Future<List<Lesson>> getLessons(String courseId, String sectionId) async {
    ...
    await firestore
        ...
        .get()
        .then((QuerySnapshot snapshot) {
      data = snapshot.docs.map((e) => Lesson.fromFirestore(e)).toList();
    });
    return data;
  }
  ...
  final DocumentReference ref = firestore.collection('users').doc(user.id);
  final String newCourseId = course.id;
  ...
  if (courses.contains(newCourseId)) {
    ...
    courses.add(newCourseId);
    await ref.update({'enrolled': FieldValue.arrayUnion([newCourseId])});
  }

  // Create subcollection 'courses_data' and document named by course id
